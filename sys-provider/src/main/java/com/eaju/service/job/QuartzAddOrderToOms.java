...
import com.eaju.pojo.OrderItem;
...
public class QuartzAddOrderToOms implements Serializable {

	private static final long serialVersionUID = -7082956986101973473L;

	private static final Logger logger = Logger.getLogger(QuartzAddOrderToOms.class);
	@Autowired
	...
	public void work() {
		...
		try {
			...
			if(findThirdPartyMerchant.size()>0){
				 ...
				 * oms订单封装
				 */
				List<OrderPojo> orderos=new ArrayList<OrderPojo>();
				for(Map.Entry<String, List<JMOrder>> map:resultMap.entrySet()){
					String key = map.getKey();
					OrderPojo orderPo=new OrderPojo();
					orderPo.setLogisticNo(key);
					orderPo.setSenderId("JM");
					orderPo.setReceiverId("OMS");
					orderPo.setUpsysNo(RandomStringUtils.randomNumeric(4));
					orderPo.setLogisticNo(key);
					//private String  customerCode;客户编码
					//private String customerName;客户名称
					//订单类型 安装-04
					orderPo.setOrderTypeCode("04");
					orderPo.setOrderTypeName("安装单");
					List<JMOrder> value = map.getValue();
					if(value.size()>0){
						JMOrder jmOrder=value.get(0);
						orderPo.setConsigneeName(jmOrder.getCustname());
						orderPo.setConsigneePhone(jmOrder.getMobile());
						orderPo.setConsigneeTel(jmOrder.getPhone());
						orderPo.setConsigneeLocationName(jmOrder.getAddress());//consigneeLocationName
						orderPo.setRemark(jmOrder.getReqnote());
						//客户代码（必填）
						orderPo.setCustomerCode("");
						//客户名称（必填）
						orderPo.setCustomerName("");
						orderPo.setDelierySetupType("00");
						orderPo.setConsigneeProvince(jmOrder.getProvince());
						orderPo.setConsigneeCity(jmOrder.getCity());
						orderPo.setConsigneeDistrictName(jmOrder.getDistrict());
						//平台编码
						orderPo.setSiteCode("");
						//平台名称
						orderPo.setSiteName("");
						//发货人名称
						orderPo.setEoorShipperContactName("");
						//发货人联系方式
						orderPo.setEoorShipperContactPhone("");
						//商家对应分公司编码
						orderPo.setCompanyCode("");
						//商家对应分公司名称
						orderPo.setCompanyName("");
						for (JMOrder jm : value) {
